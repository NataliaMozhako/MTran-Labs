...
namespace ConsoleApp1
{
    ...
    class Program
    {
        ...
        static string PrintNodeWithChildren(ExpressionNode node, string indentation)
        {
            if (node == null)
            {
                return "";
            ...
            stringBuilder.AppendLine($"{indentation} {node.Operator.Value} {node.Type}");
            if (node.Left != null)
        ...
        static void PrintSyntaxTree(IEnumerable<ExpressionNode> nodes, int nestingLevel = 1)
        {
        ...
        static void DoTheJob(IEnumerable<string> codeLines)
        {
            ...
            // semantic analysis
            SemanticErrorException semanticError = RunPython.DoSemanticCheck();
        ...
        static void Main(string[] args)
        {
            ...
            string FILENAME = @"D:/Education/Laboratory_works/МТран/Lab4/ConsoleApp1/test.py";
            ...
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($"SEMANTIC ERROR {e.ErrorType}");
                Console.ResetColor();
                Console.WriteLine($"line {e.LineNumber} char {e.PositionInLine}:");
                Console.WriteLine(errorDescription(e.PositionInLine, e.Value));
